...
/**
 * Handled screen for the Checkout Counter - the main supermarket transaction GUI
 * with 27 slots for item placement. Features keyboard hotbar shortcuts,
 * mouse scroll-based quick-move, and inventory key toggle support.
 */
public class CheckoutCounterScreen extends HandledScreen<CheckoutCounterScreenHandler> {
    private static final Identifier TEXTURE = BlockMarkMod.id("textures/gui/checkout_counter.png");
    private static final int INVENTORY_ROWS = 3;
    private static final int INVENTORY_COLS = 9;

    public CheckoutCounterScreen(CheckoutCounterScreenHandler handler, PlayerInventory inventory, Text title) {
    ...
    private void handleHotbarKeyPress(int hotbarSlot) {
        if (this.focusedSlot != null && this.focusedSlot.hasStack()) {
            int containerSlots = ((CheckoutCounterScreenHandler) this.handler).getInventory().size();
    ...
    public boolean mouseScrolled(double mouseX, double mouseY, double horizontalAmount, double verticalAmount) {
        if (this.focusedSlot != null && this.focusedSlot.hasStack() && hasShiftDown()) {
            ...
            int containerSize = ((CheckoutCounterScreenHandler) this.handler).getInventory().size();
